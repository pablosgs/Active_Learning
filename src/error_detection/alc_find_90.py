...
from src.alc_utils import corrupt_labels, invert_one_hot, alc_statistics, max_loss, rank_from_scores, retag, alc_mean_distance, knn_entropy, datamap_conf
import torch 
import random
import time
import json
...
def main():
    ...
    accs = []
    trues = []
    original_corrupted_samples = corrupted_samples
    still_missing = corrupted_samples
    percentile = 80
    total_predicted = 0
    percentiles = [90, 95, 99]
    count = 0

    while len(still_missing) > 0.2 * len(original_corrupted_samples):
        ...
        #ranked_label_issues = find_label_issues(labels=y_val_list,pred_probs=preds,return_indices_ranked_by="self_confidence", filter_by = 'prune_by_noise_rate', frac_noise=1)
        ...
        #avg_train_loss, scores = model.fit(n_epochs = 100, curriculum = 1)
        predicted_issues = rank_from_scores(scores, percentiles[count])
            
        precision, recall, accuracy, true_positives = alc_statistics(predicted_issues, still_missing, len(y))
        still_missing = np.setdiff1d(corrupted_samples, predicted_issues)
        X = np.delete(X, true_positives, axis=0)
        corrupted_y = np.delete(corrupted_y, true_positives, axis=0)
    ...
    return precisions, recalls, accs, total_predicted, percentile

if __name__ == "__main__":
    
    
    data = {str(i): [] for i in range(3)}
    final_progress = {}
    ...
    final_results.to_csv('/home/pablo/active-learning-pablo/results/test/alc/find_80/datamap.csv', index=False)
